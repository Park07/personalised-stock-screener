...
def calculate_scores(metrics, goal="value", risk="moderate"):
    ...
    if goal == "value":
        scores['pe_score'] = min(5.0, scores['pe_score'] * 1.2) if scores['pe_score'] else None
        scores['ev_ebitda_score'] = min(5.0, scores['ev_ebitda_score'] * 1.2) if scores['ev_ebitda_score'] else None
    elif goal == "income":
        scores['div_yield_score'] = min(5.0, scores['div_yield_score'] * 1.2) if scores['div_yield_score'] else None
        scores['payout_score'] = min(5.0, scores['payout_score'] * 1.2) if scores['payout_score'] else None
    elif goal == "growth":
        scores['rev_growth_score'] = min(5.0, scores['rev_growth_score'] * 1.2) if scores['rev_growth_score'] else None
        scores['earn_growth_score'] = min(5.0, scores['earn_growth_score'] * 1.2) if scores['earn_growth_score'] else None
        scores['ocf_growth_score'] = min(5.0, scores['ocf_growth_score'] * 1.2) if scores['ocf_growth_score'] else None
