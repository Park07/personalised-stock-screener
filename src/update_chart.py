...
def ensure_db_table_exists(conn):
    cursor = conn.cursor()
    try:
        # Define table with ~10-12 CORE METRICS + timestamp
        # NO score columns needed here anymore
        cursor.execute(f"""
        CREATE TABLE IF NOT EXISTS {DB_TABLE_NAME} (
            ticker TEXT PRIMARY KEY, company_name TEXT, sector TEXT,
            market_cap REAL, current_price REAL, pe_ratio REAL, roe REAL,
            dividend_yield REAL, debt_equity_ratio REAL, revenue_growth REAL,
            earnings_growth REAL, /* Add others if needed */
            last_updated TEXT
        )""")

    except Exception as e: logging.warning(f"DB Schema Ensure Error: {e}")
