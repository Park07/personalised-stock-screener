...
class TestGetValuation(unittest.TestCase):
    ...
    @patch('src.fundamentals.get_sector_pe')
    def test_get_valuation_success(self, mock_sector_pe, mock_profile, mock_growth,
        ...
        mock_sector_pe.assert_called_once_with("Technology", "2023-12-31")
    # industry pe not exist
    @patch('src.fundamentals.get_ratios')
    @patch('src.fundamentals.get_key_metrics')
    @patch('src.fundamentals.get_growth')
    @patch('src.fundamentals.get_profile')
    def test_get_valuation_missing_sector_pe(self, mock_profile, mock_growth,
                                             mock_key_metrics, mock_ratios):
        mock_ratios.return_value = {
            "date": "2023-12-31",
            "priceEarningsRatio": 20.5,
        ...
        with patch('src.fundamentals.get_sector_pe', side_effect=Exception("API error")):
            result = get_valuation("AAPL")
